#!/usr/bin/env bash
#
# specflow-state.sh - State file operations
#
# Usage:
#   specflow state get [key]              Show state or specific key
#   specflow state set <key>=<value>      Update a value
#   specflow state init [--if-missing]    Initialize new state file (--if-missing for idempotent)
#   specflow state reset                  Reset to defaults
#   specflow state validate               Validate state file
#   specflow state migrate                Migrate v1.0 state to v2.0
#   specflow state archive                Archive current phase and reset for next
#
...
readonly SPECFLOW_REGISTRY="${HOME}/.specflow/registry.json"
...
show_help() {
  cat << 'EOF'
specflow state - State file operations

USAGE:
    specflow state <command> [options]
...
COMMANDS:
    ...
    set <key>=<value>   Update a value in the state file
                        Example: specflow state set .orchestration.step=plan
...
EXAMPLES:
    ...
    specflow state get .config             # Show config section
    specflow state get .orchestration.step # Show current step
    specflow state set .project.name=MyApp # Set project name
    specflow state init                    # Create new state file
    specflow state init --if-missing       # Create only if missing (for workflows)
    specflow state validate                # Check state validity
    specflow state reconcile               # Check config vs file system
    specflow state reconcile --fix         # Fix config mismatches
    specflow state migrate                 # Upgrade to v2.0 schema
    specflow state archive                 # Archive phase, reset for next
    specflow state registry list           # List all projects
...
cmd_set() {
  ...
  # Parse key=value
  if [[ ! "$assignment" =~ ^(.+)=(.*)$ ]]; then
    log_error "Invalid format. Use: specflow state set <key>=<value>"
    exit 1
  fi
...
register_project() {
  ...
  local registry_dir
  registry_dir="$(dirname "$SPECFLOW_REGISTRY")"
  ...
  if [[ ! -f "$SPECFLOW_REGISTRY" ]]; then
    echo '{"projects": {}}' > "$SPECFLOW_REGISTRY"
  fi
  ...
  jq --arg id "$project_id" \
     ...
     "$SPECFLOW_REGISTRY" > "$temp_file"

  mv "$temp_file" "$SPECFLOW_REGISTRY"
}
...
cmd_init() {
  ...
  if [[ -f "$state_file" ]]; then
    if $if_missing; then
      ...
      log_info "Use 'specflow state init --if-missing' for idempotent init (recommended for workflows)"
      log_info "Use 'specflow state init --force' to overwrite"
      if ! confirm "Overwrite existing state file?"; then
        log_info "Keeping existing state file. Use 'specflow state set' to update values."
        exit 0
      fi
    fi
  fi
...
cmd_reset() {
  ...
  if [[ ! -f "$state_file" ]]; then
    log_error "State file not found: $state_file"
    log_info "Nothing to reset. Use 'specflow state init' to create a new state file."
    exit 1
  fi
...
cmd_validate() {
  ...
  if [[ -n "$schema_version" ]]; then
    ...
    log_warn "Legacy version field found: $legacy_version (run 'specflow state migrate')"
    ((warnings++))
  else
  ...
  for section in "${v2_sections[@]}"; do
    if ! json_has "$state_file" ".$section"; then
      log_warn "Missing v2.0 section: $section (run 'specflow state migrate')"
      ((warnings++))
    fi
  done
  ...
  if [[ -z "$project_id" || "$project_id" == "null" ]]; then
    log_warn "Missing project.id (run 'specflow state migrate')"
    ((warnings++))
  else
...
cmd_registry() {
  ...
  case "$subcommand" in
    list|ls)
      ...
      if is_json_output; then
        cat "$SPECFLOW_REGISTRY"
      else
        # Three-Line Rule: Summary first
        local project_count
        project_count=$(jq '.projects | length' "$SPECFLOW_REGISTRY" 2>/dev/null || echo "0")
        echo -e "${BLUE}INFO${RESET}: $project_count registered project(s)"
        echo ""
        # Details (line 3+)
        jq -r '.projects | to_entries[] | "  \(.value.name) (\(.key | .[0:8])...)\n    \(.value.path)"' "$SPECFLOW_REGISTRY" 2>/dev/null || echo "  (none)"
      fi
      ;;
    ...
    sync)
      ...
      project_id=$(json_get "$state_file" ".project.id" 2>/dev/null || echo "")
      if [[ -z "$project_id" || "$project_id" == "null" ]]; then
        log_error "Project has no ID - run 'specflow state migrate'"
        exit 1
      fi
      ...
      jq --arg id "$project_id" --arg ts "$timestamp" \
         '.projects[$id].last_seen = $ts' "$SPECFLOW_REGISTRY" > "$temp_file"
      mv "$temp_file" "$SPECFLOW_REGISTRY"
    ...
    clean)
      ...
      jq -r '.projects | to_entries[] | "\(.key)|\(.value.path)"' "$SPECFLOW_REGISTRY" | while IFS='|' read -r id path; do
        if [[ ! -d "$path" ]]; then
      ...
      jq -r '.projects | to_entries[] | "\(.key)|\(.value | @json)"' "$SPECFLOW_REGISTRY" 2>/dev/null | while IFS='|' read -r id value; do
        local path
      ...
      mv "$final_file" "$SPECFLOW_REGISTRY"
      log_success "Registry cleaned"
      ;;

    path)
      echo "$SPECFLOW_REGISTRY"
      ;;

    *)
      log_error "Unknown registry command: $subcommand"
      echo "Usage: specflow state registry [list|sync|clean|path]"
      exit 1
      ;;
  esac
}
...
cmd_archive() {
  ...
  if [[ ! -f "$state_file" ]]; then
    log_error "State file not found: $state_file"
    log_info "Run 'specflow state init' to create one, or 'specflow scaffold' for a new project"
    exit 1
  fi
  ...
  if [[ -z "$phase_number" || "$phase_number" == "null" ]]; then
    ...
    log_info "Use 'specflow state set orchestration.phase.number=NNN' to set a phase first"
    if is_json_output; then
  ...
  local next_name=""
  local next_json
  # Use the script's own directory to find specflow (scripts/bash -> ../../bin/specflow)
  local specflow_bin="${SCRIPT_DIR}/../../bin/specflow"
  if [[ ! -x "$specflow_bin" ]]; then
    specflow_bin="specflow"  # Fall back to PATH
  fi
  next_json=$("$specflow_bin" roadmap next --json 2>/dev/null || echo '{}')
  ...
  log_info "  2. Configure phase: specflow state set \"orchestration.phase.number=NNN\""
  log_info "  3. Update roadmap: specflow roadmap update NNN in_progress"
...
cmd_migrate() {
  ...
  if ! jq '.' "$state_file" >/dev/null 2>&1; then
    ...
    log_info "Try: specflow state init --force (will lose existing data)"
    exit 1
  fi
    ...
    jq --arg ts "$timestamp" \
       ...
       --argjson has_v1 "$has_v1_discovery" '
      {
        ...
        "config": {
          ...
          "scripts_path": (.project.scripts_path // "~/.claude/specflow-system/scripts/"),
  ...
  local manifest_script="${SCRIPT_DIR}/specflow-manifest.sh"
...
cmd_infer() {
  ...
  if [[ ! -f "$state_file" ]]; then
    log_error "State file not found. Run 'specflow state init' first."
    exit 1
  fi
...
EOF
  ...
  if [[ "$apply" == "--apply" || "$apply" == "-a" ]]; then
    ...
    if ! is_json_output; then
      echo ""
      log_info "Run 'specflow state infer --apply' to update state file"
    fi
  fi
}
...
cmd_rollback() {
  ...
  if [[ -z "$backup_file" ]]; then
    ...
    if is_json_output; then
      ...
      echo ""
      echo "To rollback: specflow state rollback <filename>"
    fi
    return 0
  fi
...
main() {
  ...
  case "$command" in
    ...
    set)
      if [[ $# -eq 0 ]]; then
        log_error "Missing argument. Usage: specflow state set <key>=<value>"
        exit 1
      fi
      cmd_set "$1"
      ;;
    ...
    *)
      ...
      echo "Run 'specflow state --help' for usage"
      exit 1
      ;;
  esac
}
