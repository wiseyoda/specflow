#!/usr/bin/env bash
#
# speckit-gate.sh - Validation gate enforcement
#
# Usage:
#   speckit gate <step>          Validate gate for step (specify, plan, tasks, implement)
#   speckit gate all             Run all gates for current state
#   speckit gate status          Show gate status summary
#
...
show_help() {
  cat << 'EOF'
speckit gate - Validation gate enforcement

USAGE:
    speckit gate <command> [options]
...
EXAMPLES:
    speckit gate specify
    speckit gate plan --strict
    speckit gate all
    speckit gate status
EOF
}
