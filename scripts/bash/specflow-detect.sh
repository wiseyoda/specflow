#!/usr/bin/env bash
#
# specflow-detect.sh - Detect existing content and documentation in a project
#
# Usage:
#   specflow detect                  Scan for all detectable patterns
#   specflow detect --check docs     Check for existing documentation
#   specflow detect --check specflow  Check for SpecFlow artifacts
#   specflow detect --check state    Check state file format/version
#
...
show_help() {
  cat << 'EOF'
specflow detect - Detect existing content and documentation

USAGE:
    specflow detect [options]
    specflow detect --check <area>
...
CHECK AREAS:
    ...
    specflow             SpecFlow project artifacts
...
EXAMPLES:
    specflow detect                  # Scan everything
    specflow detect --check docs     # Check for existing docs only
    specflow detect --json           # Output as JSON
EOF
}
...
detect_system() {
  ...
  if command_exists specflow; then
    local version
    if [[ -f "${SPECFLOW_SYSTEM_DIR}/VERSION" ]]; then
      version=$(cat "${SPECFLOW_SYSTEM_DIR}/VERSION" 2>/dev/null || echo "unknown")
    else
      version="unknown"
    fi
    print_status ok "SpecFlow CLI installed (v${version})"
    results+=("\"cli\": {\"installed\": true, \"version\": \"$version\"}")
  else
    print_status error "SpecFlow CLI not installed"
...
detect_docs() {
  ...
  if [[ -n "$adr_dir" ]]; then
    ...
    if [[ $adr_count -gt 0 ]] && ! is_json_output; then
      echo -e "  ${DIM}Import with: specflow import adrs ${rel_dir}${RESET}"
    fi
  fi
...
detect_files() {
  ...
  if [[ -f "${repo_root}/CLAUDE.md" ]]; then
    ...
    # Check if it looks like SpecFlow CLAUDE.md or user's own
    if grep -q "SpecFlow" "${repo_root}/CLAUDE.md" 2>/dev/null; then
      print_status ok "  └─ Contains SpecFlow content"
    else
      print_status warn "  └─ Custom content (not SpecFlow generated)"
    fi
  else
  ...
  if [[ -f "${repo_root}/ROADMAP.md" ]]; then
    ...
    if grep -q "## Phase" "${repo_root}/ROADMAP.md" 2>/dev/null; then
      print_status ok "  └─ SpecFlow format detected"
    fi
  else
...
detect_state() {
  ...
  if jq -e '.config' "$state_file" >/dev/null 2>&1; then
    ...
    print_status warn "  └─ Run 'specflow state migrate' to upgrade"
  fi
...
show_summary() {
  ...
  if [[ ! -d "${repo_root}/.specify" ]]; then
    project_state="new"
    recommended_action="Run /specflow.init to initialize"
  elif [[ ! -f "${repo_root}/.specify/orchestration-state.json" ]]; then
    project_state="partial"
    recommended_action="Run /specflow.init to complete setup"
  else
    ...
    if [[ "$interview_status" != "completed" ]]; then
      ...
      recommended_action="Run /specflow.init continue to resume interview"
    elif [[ ! -f "${repo_root}/ROADMAP.md" ]]; then
      project_state="needs_roadmap"
      recommended_action="Run /specflow.roadmap to create roadmap"
    elif [[ "$orch_status" == "in_progress" ]]; then
      project_state="orchestration_in_progress"
      recommended_action="Run /specflow.orchestrate continue to resume"
    else
      project_state="ready"
      recommended_action="Run /specflow.orchestrate to start next phase"
    fi
  fi
  ...
  # Check for warnings
  if [[ -f "${repo_root}/CLAUDE.md" ]] && ! grep -q "SpecFlow" "${repo_root}/CLAUDE.md" 2>/dev/null; then
    echo ""
    log_warn "Existing CLAUDE.md will be preserved - SpecFlow content will be added"
  fi

  if [[ -d "${repo_root}/docs" ]]; then
    echo ""
    log_info "Existing docs/ directory detected - SpecFlow can coexist"
  fi
}
...
main() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      *)
        ...
        echo "Run 'specflow detect --help' for usage"
        exit 1
        ;;
    esac
  done
  ...
  case "$check_area" in
    ...
    *)
      ...
      log_info "Valid areas: system, specflow, docs, files, state, all"
      exit 1
      ;;
  esac
}
