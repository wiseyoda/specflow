#!/usr/bin/env bash
#
# specflow-import.sh - Import existing documentation into SpecFlow
#
# Usage:
#   specflow import adrs <path>     Import ADRs from directory
#   specflow import adrs --dry-run  Preview import without changes
#
...
show_help() {
  cat << 'EOF'
specflow import - Import existing documentation

USAGE:
    specflow import <type> <path> [options]
...
EXAMPLES:
    ...
    specflow import adrs ./architecture/decisions --dry-run
    specflow import adrs adr --force
...
import_adrs() {
  ...
  if [[ ! -d "$source_dir" ]]; then
    ...
    log_info "Run 'specflow detect --docs' to find ADR directories"
    return 1
  fi
  ...
  case "$import_type" in
    adrs|adr)
      if [[ $# -eq 0 ]]; then
        ...
        echo "Usage: specflow import adrs <path>"
        exit 1
      fi
      import_adrs "$1"
      ;;
