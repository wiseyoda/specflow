#!/usr/bin/env bash
#
# speckit-git.sh - Git operations for SpecKit workflows
#
# Usage:
...
show_help() {
  cat << 'EOF'
speckit git - Git operations for SpecKit workflows

USAGE:
    speckit git <command> [options]
...
COMMANDS:
    ...
    branch list             List branches with SpecKit naming convention
...
EXAMPLES:
    ...
    speckit git branch checkout main
    speckit git commit "feat: add flow engine"
    speckit git merge feature-branch
    speckit git push
    speckit git sync
EOF
}
...
cmd_branch_create() {
  ...
  if [[ -z "$name" ]]; then
    ...
    echo "Usage: speckit git branch create <name>"
    exit 1
  fi
  ...
  if branch_exists_local "$name"; then
    ...
    log_info "Use 'speckit git branch checkout $name' instead"
    exit 1
  fi
...
cmd_branch_checkout() {
  ...
  if [[ -z "$name" ]]; then
    log_error "Branch name required"
    echo "Usage: speckit git branch checkout <name>"
    exit 1
  fi
  ...
  if branch_exists_local "$name"; then
    ...
    log_info "Use 'speckit git branch create $name' to create it"
    exit 1
  fi
...
cmd_commit() {
  ...
  if [[ -z "$message" ]]; then
    ...
    echo "Usage: speckit git commit <message>"
    exit 1
  fi
...
cmd_merge() {
  ...
  if [[ -z "$branch" ]]; then
    log_error "Branch name required"
    echo "Usage: speckit git merge <branch>"
    exit 1
  fi
    ...
    branch)
      ...
      case "$subcommand" in
        ...
        *)
          ...
          echo "Run 'speckit git --help' for usage"
          exit 1
          ;;
      esac
      ;;
