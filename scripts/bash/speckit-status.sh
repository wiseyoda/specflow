#!/usr/bin/env bash
#
# speckit-status.sh - Comprehensive project status for orchestration
#
# Usage:
#   speckit status              Show full status
#   speckit status --json       JSON output for orchestrate consumption
#   speckit status --quick      Skip deep validation (faster)
#
...
show_help() {
  cat << 'EOF'
speckit status - Comprehensive project status
...
EXAMPLES:
    speckit status              # Human-readable status
    speckit status --json       # JSON for orchestrate
    speckit status --quick      # Skip deep checks
EOF
}
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      *)
        ...
        echo "Run 'speckit status --help' for usage"
        exit 1
        ;;
    esac
  done
