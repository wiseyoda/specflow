#!/usr/bin/env bash
#
# specflow-dashboard.sh - Start the SpecFlow web dashboard
#
# Usage: specflow dashboard [OPTIONS]
#
# Options:
...
# Dashboard directory (relative to SpecFlow install)
...
EXAMPLES:
    specflow dashboard           # Start production server on port 4200
    specflow dashboard --dev     # Start dev server with hot reload
    specflow dashboard --port 8080  # Use port 8080
EOF
}
...
main() {
  ...
  if [[ ! -d "$DASHBOARD_DIR" ]]; then
    ...
    log_info "Ensure SpecFlow is properly installed"
    exit 1
  fi
