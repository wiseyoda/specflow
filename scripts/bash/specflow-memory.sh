#!/usr/bin/env bash
#
# specflow-memory.sh - Memory document operations
#
# Usage:
#   specflow memory init [document]     Initialize memory document(s)
#   specflow memory list                List memory documents and status
#   specflow memory check               Check memory document health
#   specflow memory path                Show memory directory path
#
...
USAGE:
    specflow memory <command> [options]
...
EXAMPLES:
    specflow memory init constitution          # Create constitution only
    specflow memory init recommended           # Create all recommended docs
    specflow memory init all                   # Create all memory docs
    specflow memory init tech-stack --force    # Overwrite existing
    specflow memory list --json                # List docs in JSON format
EOF
}
...
cmd_init() {
  ...
  if [[ -z "$doc_name" ]]; then
    ...
    echo "Usage: specflow memory init <document>"
    echo ""
...
main() {
  ...
  case "$cmd" in
    ...
    *)
      ...
      echo "Run 'specflow memory --help' for usage"
      exit 1
      ;;
  esac
}
