#!/usr/bin/env bash
#
# specflow-tasks.sh - Task tracking operations
#
# Usage:
#   specflow tasks status [file]              Count completed/total tasks
#   specflow tasks incomplete [file]          List incomplete tasks
#   specflow tasks mark <id>... [file.md]     Mark task(s) complete (supports ranges)
#   specflow tasks phase-status [file]        Status by phase
#
...
show_help() {
  cat << 'EOF'
specflow tasks - Task tracking operations

USAGE:
    specflow tasks <command> [options]
...
EXAMPLES:
    ...
    specflow tasks status specs/001-auth/tasks.md
    specflow tasks incomplete
    specflow tasks phase-status
    specflow tasks sync

    # Mark single task
    specflow tasks mark T005
    specflow tasks mark A1.1 tasks.md

    # Mark multiple tasks
    specflow tasks mark T001 T002 T003 tasks.md

    # Mark range of tasks
    specflow tasks mark T001..T010 tasks.md

    # Mix ranges and individual IDs
    specflow tasks mark A1..A3 B1 B2..B4 tasks.md

    # Start working on tasks (batch tracking)
    specflow tasks start T001 T002 --section "Setup"
    specflow tasks start A1..A5
...
cmd_mark() {
  ...
  if [[ ${#ids[@]} -eq 0 ]]; then
    ...
    echo "Usage: specflow tasks mark <id>... [file.md]"
    echo "Examples:"
    echo "  specflow tasks mark T005"
    echo "  specflow tasks mark T001 T002 T003 tasks.md"
    echo "  specflow tasks mark T001..T010 tasks.md"
    exit 1
  fi
...
cmd_start() {
  ...
  if [[ ${#ids[@]} -eq 0 ]]; then
    log_error "Task ID(s) required"
    echo "Usage: specflow tasks start <id>... [--section <name>]"
    echo "Examples:"
    echo "  specflow tasks start T001 T002 --section \"Setup\""
    echo "  specflow tasks start A1..A5"
    exit 1
  fi
  ...
  if [[ ! -f "$state_file" ]]; then
    log_error "No state file found. Run 'specflow state init' first."
    exit 1
  fi
...
cmd_working() {
  ...
  if is_json_output; then
    ...
    if [[ "$count" -eq 0 || "$current_tasks" == "[]" ]]; then
      ...
      echo "  Use 'specflow tasks start <id>...' to begin working on tasks"
    else
...
main() {
  ...
  case "$command" in
    ...
    *)
      ...
      echo "Run 'specflow tasks --help' for usage"
      exit 1
      ;;
  esac
}
