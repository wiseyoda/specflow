#!/usr/bin/env bash
#
# speckit-manifest.sh - Centralized version manifest management
#
# The manifest (.specify/manifest.json) is the single source of truth for
# all version information in a SpecKit project.
#
# Usage:
#   speckit manifest init              Initialize manifest for project
#   speckit manifest get [key]         Get manifest value(s)
#   speckit manifest set <key> <val>   Set manifest value
#   speckit manifest validate          Validate manifest and compatibility
#   speckit manifest upgrade           Check for and apply upgrades
#
...
readonly SPECKIT_VERSION="2.1.0"
...
show_help() {
  cat << 'EOF'
speckit manifest - Centralized version manifest management

USAGE:
    speckit manifest <command> [options]
...
MANIFEST KEYS:
    speckit_version     SpecKit system version
...
EXAMPLES:
    ...
    speckit manifest get                # Show all versions
    speckit manifest get roadmap_format # Get specific version
...
cmd_get() {
  ...
  if [[ ! -f "$manifest_path" ]]; then
    ...
    log_info "Run 'speckit manifest init' to create"
    exit 1
  fi
  ...
  if [[ -z "$key" ]]; then
    ...
    if is_json_output; then
      ...
      echo "SpecKit Manifest"
      echo "================"
      echo ""
      echo "Core Version:     $(jq -r '.speckit_version' "$manifest_path")"
      echo ""
    ...
    local value
    case "$key" in
      speckit_version|speckit)
        value=$(jq -r '.speckit_version' "$manifest_path")
        ;;
      state_schema|state)
...
cmd_set() {
  ...
  case "$key" in
    ...
    *)
      log_error "Unknown key: $key"
      log_info "Valid keys: speckit_version, state_schema, roadmap_format, commands_version, min_cli"
      exit 1
      ;;
  esac
...
cmd_validate() {
  ...
  if [[ -n "$min_cli" ]]; then
    if version_gte "$SPECKIT_VERSION" "$min_cli"; then
      print_status ok "CLI compatible (${SPECKIT_VERSION} >= ${min_cli})"
    else
      issues+=("CLI version $SPECKIT_VERSION < required $min_cli")
      print_status error "CLI too old: ${SPECKIT_VERSION} < ${min_cli}"
    fi
  fi
  ...
  current_version=$(jq -r '.speckit_version // ""' "$manifest_path")
  if [[ -n "$current_version" ]]; then
    if version_gte "$SPECKIT_VERSION" "$current_version"; then
      if [[ "$SPECKIT_VERSION" != "$current_version" ]]; then
        warnings+=("Upgrade available: $current_version -> $SPECKIT_VERSION")
        print_status warn "Upgrade available: $current_version -> $SPECKIT_VERSION"
      else
        print_status ok "SpecKit version: $current_version (current)"
      fi
    else
      warnings+=("Project uses newer SpecKit: $current_version (CLI: $SPECKIT_VERSION)")
...
cmd_upgrade() {
  ...
  echo "Latest version:  $SPECKIT_VERSION"
  echo ""

  if [[ "$current_version" == "$SPECKIT_VERSION" ]]; then
    log_success "Already at latest version"
    return 0
  fi

  if ! version_gte "$SPECKIT_VERSION" "$current_version"; then
    log_warn "Project uses newer SpecKit ($current_version)"
    log_info "Update your SpecKit installation first"
    return 1
  fi
  ...
  if [[ ${#upgrades_needed[@]} -eq 0 ]]; then
    ...
    # Just update version
    record_migration "$current_version" "$SPECKIT_VERSION" "Version bump only"

    local temp_file
    temp_file=$(mktemp)
    jq ".speckit_version = \"$SPECKIT_VERSION\"" "$manifest_path" > "$temp_file"
    mv "$temp_file" "$manifest_path"

    log_success "Updated to version $SPECKIT_VERSION"
    return 0
  fi
  ...
  record_migration "$current_version" "$SPECKIT_VERSION" "Applied: ${upgrades_needed[*]}"
  ...
  log_success "Upgraded to version $SPECKIT_VERSION"
}
...
cmd_status() {
  ...
  if [[ ! -f "$manifest_path" ]]; then
    ...
    echo ""
    echo "Run 'speckit manifest init' to create"
    exit 1
  fi

  local project_version
  project_version=$(jq -r '.speckit_version // "unknown"' "$manifest_path")

  echo "SpecKit Version Status"
  echo "======================"
  echo ""
  echo "CLI Version:     $SPECKIT_VERSION"
  echo "Project Version: $project_version"
  echo ""

  if [[ "$project_version" == "$SPECKIT_VERSION" ]]; then
    echo "Status: UP TO DATE"
  elif version_gte "$SPECKIT_VERSION" "$project_version"; then
    echo "Status: UPGRADE AVAILABLE"
    echo ""
    echo "Run 'speckit manifest upgrade' to upgrade"
  else
    echo "Status: PROJECT NEWER THAN CLI"
    echo ""
    echo "Update your SpecKit installation"
  fi
...
main() {
  ...
  case "$command" in
    ...
    *)
      ...
      echo "Run 'speckit manifest --help' for usage"
      exit 1
      ;;
  esac
}
