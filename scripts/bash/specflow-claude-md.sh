#!/usr/bin/env bash
#
# specflow-claude-md.sh - CLAUDE.md operations
#
# Usage:
#   specflow claude-md update <description>   Add to Recent Changes
#   specflow claude-md sync                   Sync from ROADMAP.md completions
#   specflow claude-md init                   Initialize Recent Changes section
#
...
show_help() {
  cat << 'EOF'
specflow claude-md - CLAUDE.md operations

USAGE:
    specflow claude-md <command> [options]
...
COMMANDS:
    ...
    merge [--dry-run]   Merge SpecFlow sections into existing CLAUDE.md
                        Preserves user content, adds SpecFlow sections
...
EXAMPLES:
    specflow claude-md update "Phase 002: Added flow engine with state machine"
    specflow claude-md sync
    specflow claude-md merge --dry-run
    specflow claude-md show
EOF
}
...
cmd_update() {
  ...
  if [[ -z "$description" ]]; then
    ...
    echo "Usage: specflow claude-md update <description>"
    exit 1
  fi
...
cmd_show() {
  ...
  if ! has_recent_changes_section; then
    ...
    log_info "Run 'specflow claude-md init' to create one"
    exit 0
  fi
...
cmd_merge() {
  ...
  # Check if SpecFlow section already exists
  if grep -qF "## SpecFlow" "$claude_path" 2>/dev/null; then
    log_info "SpecFlow section already present in CLAUDE.md"
    if is_json_output; then
  ...
  echo "  SpecFlow section: missing"
  echo ""

  if $dry_run; then
    log_info "[DRY RUN] Would add minimal SpecFlow section (~7 lines)"
    echo ""
    echo "  ## SpecFlow"
    echo "  This project uses SpecFlow for spec-driven development."
    echo "  **Quick Start**: /specflow.start"
    echo "  **CLI Help**: specflow --help"
    echo "  **Full Reference**: .specify/USAGE.md"

    if is_json_output; then
      echo '{"dry_run": true, "would_add": ["SpecFlow"]}'
    fi
    return 0
  fi
  ...
  # Minimal SpecFlow section content (≤15 lines as per UX simplification)
  local specflow_section
  specflow_section="
## SpecFlow

This project uses SpecFlow for spec-driven development.

**Quick Start**: \`/specflow.start\` - auto-routes to the right command
...
"
  ...
  # Strategy: Insert SpecFlow section after the main header/description
  # Look for the first ## heading and insert before it
  awk -v section="$specflow_section" '
    /^## / && !inserted {
      printf "%s", section
      inserted = 1
    }
  ...
  log_success "Added minimal SpecFlow section to CLAUDE.md"
...
main() {
  ...
  case "$command" in
    ...
    *)
      ...
      echo "Run 'specflow claude-md --help' for usage"
      exit 1
      ;;
  esac
}
