...
```bash
# Discovery artifacts (from /specflow.init)
...
```bash
specflow phase create 0010 "phase-name"
```
...
3. **Update orchestration state** (if exists) using CLI:
   ```bash
   specflow state set "config.roadmap_path=ROADMAP.md"
   ```

4. **Update CLAUDE.md** with the new roadmap:
   ```bash
   specflow claude-md update "ROADMAP.md" "Created project roadmap with N phases"
   ```
...
This command uses the SpecFlow CLI (`specflow`) for ROADMAP operations:
...
- `specflow roadmap` - ROADMAP operations (status, validate, current, next)
- `specflow state` - State management (set config path)
- `specflow claude-md` - CLAUDE.md updates (update)

## Integration with SpecFlow Flow

**Where ROADMAP.md fits:**

```
/specflow.init          → Creates discovery artifacts, constitution, memory docs, and ROADMAP
/specflow.roadmap       → Creates/updates ROADMAP.md (THIS COMMAND)
/specflow.roadmap add-pdr → Converts PDRs to ROADMAP phases
/specflow.roadmap backlog → Triages backlog items into phases
/specflow.orchestrate   → Reads ROADMAP.md, executes phases
/specflow.design        → Creates spec, plan, tasks for current phase
```
...
```
/specflow.roadmap add-pdr                    # List available PDRs
/specflow.roadmap add-pdr pdr-feature.md     # Convert specific PDR
/specflow.roadmap add-pdr pdr-a.md pdr-b.md  # Convert multiple PDRs
/specflow.roadmap add-pdr --all              # Convert all approved PDRs
```
...
specflow pdr list --json

# Show specific PDR details
specflow pdr show <filename>
```
...
- Or insert after specific phase: `specflow roadmap insert --after 0020`
...
**Deliverables**:
<!-- To be determined during /specflow.design -->
...
```bash
# For each new phase
specflow roadmap insert --after NNNN "Phase Name" --non-interactive
```
...
- `specflow pdr list` shows only unprocessed PDRs
- `specflow pdr list --all` shows all PDRs including processed ones
...
PDRs marked as processed:
  specflow pdr mark pdr-offline-mode.md  ✓
  specflow pdr mark pdr-sync-status.md   ✓

Next steps:
  1. Review ROADMAP.md for accuracy
  2. Run /specflow.orchestrate to begin development
  3. Or run /specflow.design to create detailed spec for first phase
...
To approve a PDR:
  ...
  2. Run: specflow pdr validate pdr-feature.md
...
specflow pdr validate <filename>
```
...
# PDR operations
specflow pdr list --json
specflow pdr show <file>
specflow pdr validate <file>
specflow pdr mark <file>      # Mark PDR as processed after phase creation

# Roadmap operations
specflow roadmap status --json
specflow roadmap next --json
specflow roadmap insert --after <phase> "<name>"
specflow roadmap validate
```
...
```
/specflow.roadmap backlog                    # Interactive triage on all backlog items
/specflow.roadmap backlog --auto             # Auto-assign high-confidence matches
/specflow.roadmap backlog --dry-run          # Preview assignments without changes
```
...
specflow roadmap status --json | jq -r '.phases[] | select(.status == "complete") | .number'
```
...
1. Add to backlog: `specflow roadmap backlog add "[Orphaned from NNNN] T###: Task description"`
2. Mark original as deferred: `- [x] T### ... *(deferred to backlog)*`

#### Step 2: Parse Backlog Items

```bash
BACKLOG_JSON=$(specflow roadmap backlog list --json)
...
For new phases:
```bash
specflow roadmap insert --after "$LAST_PHASE" "$NEW_PHASE_NAME" --non-interactive
```
...
Run /specflow.orchestrate to continue development
```
...
| No phases defined | "Create phases first with /specflow.roadmap" |
