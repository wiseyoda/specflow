#!/usr/bin/env bash
#
# specflow-issue.sh - Local issue tracking operations
#
# Usage:
...
show_help() {
  cat << 'EOF'
specflow issue - Local issue tracking operations

USAGE:
    specflow issue <command> [options]
...
COMMANDS:
    ...
    show <id>           Show issue details
                        Example: specflow issue show ISSUE-001
...
EXAMPLES:
    specflow issue list --open
    specflow issue show ISSUE-003
    specflow issue create "Fix login timeout" --category bug --priority high
    specflow issue close ISSUE-003 --resolution "Fixed in phase 0042"
    specflow issue migrate --dry-run
EOF
}
...
cmd_list() {
  ...
  if [[ ! -d "$issues_dir" ]]; then
    if is_json_output; then
      ...
      echo "Create issues with: specflow issue create \"<title>\""
    fi
    exit 0
  fi
...
cmd_show() {
  ...
  if [[ -z "$id" ]]; then
    ...
    echo "Usage: specflow issue show <id>"
    exit 1
  fi
  ...
  if [[ -z "$file" ]]; then
    ...
    echo "List issues with: specflow issue list"
    exit 1
  fi
...
cmd_create() {
  ...
  if [[ -z "$title" ]]; then
    ...
    echo "Usage: specflow issue create \"<title>\" [--category bug] [--priority high]"
    exit 1
  fi
...
EOF
  ...
  if is_json_output; then
    ...
    echo "View: specflow issue show $id"
  fi
}
...
cmd_close() {
  ...
  if [[ -z "$id" ]]; then
    log_error "Issue ID required"
    echo "Usage: specflow issue close <id> [--resolution \"notes\"]"
    exit 1
  fi
...
cmd_update() {
  ...
  if [[ -z "$id" ]]; then
    log_error "Issue ID required"
    echo "Usage: specflow issue update <id> [--status open] [--phase 0050]"
    exit 1
  fi
...
EOF
  ...
  echo ""
  echo "Review with: specflow issue list"
  echo ""
...
main() {
  ...
  case "$command" in
    ...
    *)
      ...
      echo "Run 'specflow issue --help' for usage"
      exit 1
      ;;
  esac
}
