#!/usr/bin/env bash
#
# specflow-templates.sh - Template versioning and management
#
# Usage:
#   specflow templates check           Compare versions, show outdated
#   specflow templates update [file]   Update specific template
#   specflow templates update-all      Update all templates
#   specflow templates diff [file]     Show differences
...
readonly SYSTEM_TEMPLATES="${SPECFLOW_SYSTEM_DIR}/templates"
...
show_help() {
  cat << 'EOF'
specflow templates - Template versioning and management

USAGE:
    specflow templates <command> [options]
...
EXAMPLES:
    ...
    specflow templates update spec-template.md
    specflow templates diff plan-template.md
...
cmd_check() {
  ...
  if is_json_output; then
    ...
    if [[ $outdated -gt 0 ]]; then
      ...
      log_info "Run 'specflow templates update-all' to update"
    else
...
cmd_update() {
  ...
  if [[ -z "$filename" ]]; then
    ...
    echo "Usage: specflow templates update <file>"
    exit 1
  fi
...
cmd_diff() {
  ...
  if [[ -z "$filename" ]]; then
    log_error "Template filename required"
    echo "Usage: specflow templates diff <file>"
    exit 1
  fi
  ...
  if [[ ! -f "$proj_template" ]]; then
    ...
    log_info "Run 'specflow templates copy $filename' to create it"
    exit 1
  fi
...
cmd_copy() {
  ...
  if [[ -z "$filename" ]]; then
    log_error "Template filename required"
    echo "Usage: specflow templates copy <file>"
    exit 1
  fi
...
main() {
  ...
  case "$command" in
    ...
    *)
      ...
      echo "Run 'specflow templates --help' for usage"
      exit 1
      ;;
  esac
}
