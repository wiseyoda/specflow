...
**Independent Test**: Can be fully tested by running `specflow roadmap insert --after 0020 "Urgent Fix"` and verifying a new phase 0021 is created with proper formatting.

**Acceptance Scenarios**:

1. **Given** a ROADMAP.md with phases 0010, 0020, 0030, **When** user runs `specflow roadmap insert --after 0020 "Hotfix Auth Bug"`, **Then** a new phase 0021 is created between 0020 and 0030 with proper table entry and section
...
**Independent Test**: Can be fully tested by running `specflow roadmap defer 0040` and verifying the phase moves to a Backlog section with its details preserved.

**Acceptance Scenarios**:

1. **Given** a ROADMAP.md with active phase 0040, **When** user runs `specflow roadmap defer 0040`, **Then** phase 0040 is removed from the Phase Overview table and added to a Backlog section
2. **Given** no Backlog section exists, **When** user defers a phase, **Then** a Backlog section is created before the phase is added
3. **Given** a deferred phase in Backlog, **When** user runs `specflow roadmap restore 0040 --after 0030`, **Then** the phase is restored to the active roadmap (renumbered as needed)
...
**Independent Test**: Can be fully tested by running `specflow migrate roadmap` on a 2.0 format ROADMAP.md and verifying 001→0010, 002→0020 conversion.

**Acceptance Scenarios**:

1. **Given** a ROADMAP.md with phases 001, 002, 003, **When** user runs `specflow migrate roadmap`, **Then** phases become 0010, 0020, 0030 and all internal references are updated
2. **Given** a ROADMAP.md already in 2.1 format, **When** user runs `specflow migrate roadmap`, **Then** no changes are made and user is informed the roadmap is current
3. **Given** a mixed format ROADMAP.md, **When** user runs `specflow migrate roadmap`, **Then** an error is shown listing the inconsistencies
...
- **FR-002**: System MUST provide `specflow roadmap insert --after <phase> "<name>"` command to create new phases
- **FR-003**: System MUST provide `specflow roadmap defer <phase>` command to move phases to Backlog
- **FR-004**: System MUST provide `specflow roadmap restore <phase> --after <phase>` command to restore from Backlog
- **FR-005**: System MUST maintain a Backlog section in ROADMAP.md for deferred phases
- **FR-006**: System MUST provide `specflow migrate roadmap` command to convert 2.0 → 2.1 format
...
- **SC-001**: `specflow roadmap insert --after 0020 "Urgent Fix"` creates phase 0021 in under 2 seconds
- **SC-002**: `specflow roadmap defer 0040` moves phase to Backlog preserving all content
- **SC-003**: `specflow migrate roadmap` converts all 3-digit phases to 4-digit format with zero data loss
