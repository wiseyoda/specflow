#!/usr/bin/env bash
#
# specflow-pdr.sh - PDR (Product Design Requirements) operations
#
# Usage:
#   specflow pdr list              List all PDRs with status
#   specflow pdr status            Show PDR summary
...
show_help() {
  cat << 'EOF'
specflow pdr - PDR (Product Design Requirements) operations

USAGE:
    specflow pdr <command> [options]
...
EXAMPLES:
    specflow pdr list
    specflow pdr list --all
    specflow pdr status
    specflow pdr show pdr-offline-mode.md
    specflow pdr validate pdr-offline-mode.md
...
ensure_pdr_dir() {
  ...
  if [[ ! -d "$pdr_path" ]]; then
    ...
    log_info "Create it with: specflow pdr init"
    exit 1
  fi
}
  ...
  if [[ ${#pdr_files[@]} -eq 0 ]]; then
    if is_json_output; then
      ...
      if [[ ${#processed_files[@]} -gt 0 ]]; then
        ...
        log_info "Use 'specflow pdr list --all' to see processed PDRs"
      else
...
cmd_status() {
  ...
  if is_json_output; then
    ...
    if [[ $approved_count -gt 0 ]]; then
      echo ""
      log_info "$approved_count PDR(s) ready for phase creation: /specflow.phase"
    fi
  fi
}
...
cmd_show() {
  ...
  if [[ -z "$file" ]]; then
    ...
    echo "Usage: specflow pdr show <file>"
    exit 1
  fi
...
cmd_validate() {
  ...
  if [[ -z "$file" ]]; then
    log_error "File required"
    echo "Usage: specflow pdr validate <file>"
    exit 1
  fi
...
cmd_mark() {
  ...
  if [[ -z "$file" ]]; then
    log_error "File required"
    echo "Usage: specflow pdr mark <file>"
    exit 1
  fi
...
cmd_unmark() {
  ...
  if [[ -z "$file" ]]; then
    log_error "File required"
    echo "Usage: specflow pdr unmark <file>"
    exit 1
  fi
...
main() {
  ...
  case "$command" in
    ...
    *)
      ...
      echo "Run 'specflow pdr --help' for usage"
      exit 1
      ;;
  esac
}
