...
1. **NEVER edit `tasks.md` directly to mark tasks complete** - Use `specflow tasks mark T###`
2. **NEVER edit `.specify/orchestration-state.json` directly** - Use `specflow state set`
3. **ALWAYS use `specflow tasks mark`** after completing each task
4. **ALWAYS use `specflow tasks status`** to check progress
...
1. Run `specflow context --json --require-tasks --include-tasks` from repo root and parse FEATURE_DIR and AVAILABLE_DOCS list. All paths must be absolute. For single quotes in args like "I'm Groot", use escape syntax: e.g 'I'\''m Groot' (or double-quote if possible: "I'm Groot").

2. **Update State**: Mark step as in-progress:
   ```bash
   specflow state set "orchestration.step.current=implement" "orchestration.step.status=in_progress"
   ```
...
6. Execute implementation following the task plan:
   ...
   - **Track in-progress tasks**: Before starting work on a batch/section of tasks, mark them as in-progress:
     ...
     specflow tasks start T001 T002 T003 --section "Phase 1: Setup"

     # Check what's currently being worked on
     specflow tasks working
     ```
...
8. Progress tracking and error handling:
   ...
   - **IMPORTANT** Use the SpecFlow CLI to mark completed tasks:

     ```bash
     # Mark individual task as complete (also updates state file + dashboard)
     specflow tasks mark T001
     specflow tasks mark T002
     ...
     The `specflow tasks mark` command automatically:
...
9. **Update State**: Mark step as complete (or failed on error):
   ...
   **Error Handling**: If implementation fails (tests consistently failing, blocking errors), mark step as `failed`. The dashboard will show this state, and `/specflow.orchestrate` can attempt recovery.

10. Completion validation:

   Use the SpecFlow CLI to verify completion:
...
This command uses the SpecFlow CLI (`specflow`) for implementation tracking:
...
- `specflow checklist` - Pre-implementation checklist verification (status, list, incomplete)
- `specflow tasks` - Task tracking during implementation (status, mark, incomplete, phase-status)
- `specflow state` - State file updates (automatic via tasks mark)

Note: This command assumes a complete task breakdown exists in tasks.md. If tasks are incomplete or missing, suggest running `/specflow.design --tasks` first to regenerate the task list.
