#!/usr/bin/env bash
#
# specflow-lessons.sh - Lessons learned management
#
# Usage:
#   specflow lessons init [file]           Initialize lessons-learned.md
#   specflow lessons add <type> <desc>     Add an entry (error, decision, gotcha)
#   specflow lessons check <keyword>       Search lessons for keyword
#   specflow lessons list                  List all entries
#   specflow lessons show [file]           Show lessons file
#
...
show_help() {
  cat << 'EOF'
specflow lessons - Lessons learned management

USAGE:
    specflow lessons <command> [options]
...
EXAMPLES:
    specflow lessons init
    specflow lessons add error "Database connection timeout"
    specflow lessons add decision "Chose Redis for caching"
    specflow lessons add gotcha "macOS sed" "No -i flag" "Use temp file"
    specflow lessons check "timeout"
    specflow lessons list
EOF
}
...
cmd_add() {
  ...
  if [[ -z "$file" || ! -f "$file" ]]; then
    ...
    log_info "Run 'specflow lessons init' first"
    exit 1
  fi
...
"
    ...
    gotcha)
      ...
      if [[ -z "$tech" || -z "$issue" ]]; then
        log_error "Usage: specflow lessons add gotcha <technology> <issue> [workaround]"
        exit 1
      fi
      # Add to gotcha table
      local temp_file
...
main() {
  ...
  case "$command" in
    ...
    *)
      ...
      echo "Run 'specflow lessons --help' for usage"
      exit 1
      ;;
  esac
}
