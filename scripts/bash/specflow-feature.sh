#!/usr/bin/env bash
#
# specflow-feature.sh - Feature operations
#
# Usage:
#   specflow feature create <phase> <name>    Create new feature directory
#   specflow feature list                     List all features
#   specflow feature status                   Show feature status summary
#
...
show_help() {
  cat << 'EOF'
specflow feature - Feature operations

USAGE:
    specflow feature <command> [options]
...
EXAMPLES:
    specflow feature create 0010 project-setup
    specflow feature create 0020 core-engine --no-branch
    specflow feature list
    specflow feature list --json
    specflow feature status
EOF
}
...
get_spec_template() {
  local system_dir
  system_dir="$(get_specflow_system_dir)"
...
cmd_create() {
  ...
  if [[ -z "$phase" ]] || [[ -z "$name" ]]; then
    ...
    echo "Usage: specflow feature create <phase> <name>"
    echo "Example: specflow feature create 0010 project-setup"
    exit 1
  fi
...
EOF
  ...
  echo "  2. Run /specflow.specify to elaborate the spec"
  echo "  3. Run /specflow.plan to create implementation plan"
...
main() {
  ...
  case "$command" in
    ...
    *)
      ...
      echo "Run 'specflow feature --help' for usage"
      exit 1
      ;;
  esac
}
