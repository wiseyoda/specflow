#!/usr/bin/env bash
#
# specflow-context.sh - Get current feature context
#
...
#
# Usage:
#   specflow context              Show current feature context
#   specflow context --json       Output as JSON
#   specflow context --require-spec   Require spec.md to exist
#   specflow context --require-plan   Require plan.md to exist
#   specflow context --require-tasks  Require tasks.md to exist
#
...
show_help() {
  cat << 'EOF'
specflow context - Get current feature context

USAGE:
    specflow context [options]
...
EXAMPLES:
    ...
    specflow context --require-plan --json
    specflow context --require-tasks
EOF
}
...
cmd_context() {
  ...
  if [[ -z "$feature_dir" ]] || [[ ! -d "$feature_dir" ]]; then
    if is_json_output; then
      ...
      log_info "Create with: /specflow.specify or specflow feature create"
    fi
    exit 1
  fi
  ...
  if $require_spec && [[ "$DOC_SPEC" == "false" ]]; then
    if is_json_output; then
      ...
      log_info "Run /specflow.specify to create the specification"
    fi
    exit 1
  fi
  ...
  if $require_plan && [[ "$DOC_PLAN" == "false" ]]; then
    if is_json_output; then
      ...
      log_info "Run /specflow.plan to create the implementation plan"
    fi
    exit 1
  fi
  ...
  if $require_tasks && [[ "$DOC_TASKS" == "false" ]]; then
    if is_json_output; then
      ...
      log_info "Run /specflow.tasks to create the task list"
    fi
    exit 1
  fi
