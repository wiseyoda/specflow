#!/usr/bin/env bash
#
# specflow-roadmap.sh - ROADMAP.md operations
#
# Usage:
#   specflow roadmap status              Show all phase statuses
#   specflow roadmap update <phase> <s>  Update phase status
...
show_help() {
  cat << 'EOF'
specflow roadmap - ROADMAP.md operations

USAGE:
    specflow roadmap <command> [options]
...
EXAMPLES:
    ...
    specflow roadmap update 0020 in_progress
    specflow roadmap insert --after 0020 "Hotfix Auth"
    specflow roadmap defer 0040
    specflow roadmap restore 0040 --after 0030
    specflow roadmap renumber --dry-run
...
ensure_roadmap() {
  ...
  if [[ ! -f "$roadmap_path" ]]; then
    ...
    log_info "Create one with: /specflow.roadmap"
    exit 1
  fi
}
...
cmd_update() {
  ...
  if [[ -z "$phase" ]] || [[ -z "$new_status" ]]; then
    ...
    echo "Usage: specflow roadmap update <phase> <status>"
    exit 1
  fi
...
cmd_insert() {
  ...
  if [[ -z "$after_phase" ]] || [[ -z "$phase_name" ]]; then
    ...
    echo "Usage: specflow roadmap insert --after <phase> \"<name>\""
    echo "Example: specflow roadmap insert --after 0020 \"Hotfix Auth Bug\""
    exit 1
  fi
  ...
  if [[ -z "$new_phase" ]]; then
    ...
    echo "Consider deferring some phases: specflow roadmap defer <phase>"
    exit 1
  fi
...
cmd_defer() {
  ...
  if [[ -z "$phase" ]]; then
    ...
    echo "Usage: specflow roadmap defer <phase> [--force]"
    exit 1
  fi
...
cmd_restore() {
  ...
  if [[ -z "$phase" ]]; then
    log_error "Phase number required"
    echo "Usage: specflow roadmap restore <phase> [--after <phase>] [--as <number>]"
    exit 1
  fi
  ...
  if ! grep -qE "^\|[[:space:]]*${phase}[[:space:]]*\|" "$roadmap_path" | grep -q "Deferred"; then
    ...
    if ! $in_backlog; then
      ...
      echo "Use 'specflow roadmap status' to see available phases"
      exit 1
    fi
  fi
...
cmd_backlog() {
  ...
  case "$action" in
    ...
    *)
      ...
      echo "Run 'specflow roadmap backlog help' for usage"
      exit 1
      ;;
  esac
}
...
cmd_backlog_help() {
  cat << 'EOF'
specflow roadmap backlog - Manage ROADMAP.md backlog items

USAGE:
    specflow roadmap backlog <action> [options]
...
EXAMPLES:
    specflow roadmap backlog add "Add dark mode support"
    specflow roadmap backlog add "Improve error messages"
    specflow roadmap backlog list
    specflow roadmap backlog clear
EOF
}
...
cmd_backlog_add() {
  if [[ $# -eq 0 ]]; then
    ...
    echo "Usage: specflow roadmap backlog add \"<item>\""
    exit 1
  fi
...
Items captured for future triage. Run `/specflow.backlog` to assign to phases.
...
main() {
  ...
  case "$command" in
    ...
    *)
      ...
      echo "Run 'specflow roadmap --help' for usage"
      exit 1
      ;;
  esac
}
