#!/usr/bin/env bash
#
# specflow-migrate.sh - Migration utilities for SpecFlow
#
# Usage:
#   specflow migrate roadmap            Migrate ROADMAP.md from 2.0 to 2.1 format
#   specflow migrate roadmap --dry-run  Preview changes without applying
#
...
show_help() {
  cat << 'EOF'
specflow migrate - Migration utilities for SpecFlow

USAGE:
    specflow migrate <target> [options]
...
EXAMPLES:
    specflow migrate roadmap              # Migrate and create backup
    specflow migrate roadmap --dry-run    # Preview changes only
EOF
}
...
main() {
  ...
  case "$target" in
    ...
    *)
      ...
      echo "Run 'specflow migrate --help' for usage"
      exit 1
      ;;
  esac
}
